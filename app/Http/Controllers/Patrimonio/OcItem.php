...
class OcItem extends Controller
{
         ...
         $datos_items = null;  
          ...
          foreach ($datos as $d) {//recorremos y chekeamos si escompatible SIGEUN CON SIGA
           ...
           //EXTRAEMOS el clasificador
            $cuentas=array();
            $clasificador_='';
            $clasificador = \DB::connection('sqlsrvSiga')->select("EXEC dbo.SP_SIGEUN_PAT_SEL_OC_ITEM_PPTO ?,?,?,?,?",array( $_SEC_EJE,$_anio,$d->NRO_ORDEN,$d->SEC_ITEM,'B'));
            if($clasificador){
  ...
  public function comboCuentas($_anio,$clasificador,$GRUPO_BIEN,$CLASE_BIEN,$FAMILIA_BIEN){  
          $cuentas_data=array();
          $cuentas =  \DB::connection('sqlsrvSiga')->select("EXEC dbo.SP_SIGEUN_PAT_SEL_OC_ITEM_PPTO_CUENTAS ?,?,?,?,?",array( $_anio,$clasificador, $GRUPO_BIEN,$CLASE_BIEN,$FAMILIA_BIEN));
          if($cuentas){
