...
class Oc extends Controller
{
    ...
    //public function getResult($skip,$top,$inlinecount,$format){
     public function getResult(Request $request){
          ...
          $datos = null;        
         $datos = \DB::connection('sqlsrvSiga')->select("EXEC dbo.SP_SIGEUN_PAT_SEL_OC ?,?,?,?,?",array( $skip,$top,$_SEC_EJE, $_anio,$value_filtro_cDocAdqNro));
          
     // print_r($datos)
         // $total=45;
          $total=\DB::connection('sqlsrvSiga')->select("EXEC dbo.SP_SIGEUN_PAT_COUNT_OC ?,?,?",array($value_filtro_cDocAdqNro, $_anio,$_SEC_EJE));
          $data = [       
